# SPS Recent Results — St. Paul's School (Big Red), every team: Varsity,
# JV, and JV-B/Thirds.
#
# Companion app to sps-athletics-upcoming (same data, opposite half): this
# one shows only games that HAVE been played, score intact, staying up all
# week — it only rolls to the next week's results the following Monday, per
# ctx.now. A game with no result yet is on the Upcoming app instead.
# Splitting into two apps this way, rather than one page per day per app, is
# what makes room for both "what's coming up" and "what just happened" to
# each get a full week's worth of pages instead of fighting over one shared
# set of 8 (see the page-count note below).
#
...
# set the device always cycles through, and a "no result" placeholder on
# every day without one would defeat the point of the app). So instead,
# week_matches() gathers the current week's single best played game for
# every day that actually has a result — skipping days with none entirely —
# and each of the 7 slots picks one modulo that count. A light week (say 3
# results so far) cycles those 3 across all 7 slots rather than padding with
# blanks; a heavy week just shows 7 different results. Only a genuinely
# resultless week falls back to an explicit empty state.
#
...
# them are the result slots above; the 8th is a "this week" cover page. On
...
def events_for_day(events, y, m, d, want_played):
    ...
    `want_played` — a day with a doubleheader, or several different sports
    all playing the same day, produces one entry per game, not just the
    single highest-priority one. Sorted lowest level_rank first, then
    earliest time."""
    target = fmt.pad(y, 4) + "-" + fmt.pad(m) + "-" + fmt.pad(d)
    day_list = []
    for e in events:
        if e["date"][0:10] == target and has_result(e) == want_played:
            day_list.append(e)
...
def clip_words(c, text, font, maxw):
    """Like clip(), but backs up to the last whole word — a long name
    should drop its last word entirely rather than mangle it mid-word. Only
    backs up when that costs at most 30% of the string, so a single very
    long word still gets a hard cut instead of vanishing to nothing."""
...
def side_box(c, x, y, name):
    ...
    # enough left to run into that same row.
...
def week_matches(ctx, want_played):
    ...
    equivalent to "don't show empty days" is this: compact the actual
    results to the front, and let a page pick modulo the match count so
    extra slots cycle back through real results instead of ever showing a
    blank — and so a busy week's extra results still get their turn across
    the rotation instead of being silently dropped."""
...
def render_results_slot(c, ctx, slot_index):
    matches, any_ok = week_matches(ctx, want_played = True)
    if not any_ok:
        empty_card(c, "THIS WEEK", ["FEED", "UNAVAILABLE"])
        return
    if len(matches) == 0:
        empty_card(c, "THIS WEEK", ["NO RESULTS", "YET"])
        return
    ...
    if result["team_score"] != None and result["opponent_score"] != None:
        ...
        lines = [when, "FINAL"]
    card(c, ctx, header, e["opponent"]["title"].strip(),
         lines, accent = accent, small_line = result_note(result), icon_title = e["sport"]["title"])

def this_week(c, ctx):
    monday = week_monday_days(ctx)
    y1, m1, d1 = date_from_days(monday)
    y2, m2, d2 = date_from_days(monday + 6)
    y0 = c.header("RECENT RESULTS", bg = BIG_RED, color = "black", font = "5x7")
    c.image("logos/sps.png", 2, y0, ICON, ICON)
    cx = c.width // 2 + ICON // 2
    if m1 == m2:
...
def slot1(c, ctx):
    render_results_slot(c, ctx, 0)

def slot2(c, ctx):
    render_results_slot(c, ctx, 1)

def slot3(c, ctx):
    render_results_slot(c, ctx, 2)

def slot4(c, ctx):
    render_results_slot(c, ctx, 3)

def slot5(c, ctx):
    render_results_slot(c, ctx, 4)

def slot6(c, ctx):
    render_results_slot(c, ctx, 5)

def slot7(c, ctx):
    render_results_slot(c, ctx, 6)
